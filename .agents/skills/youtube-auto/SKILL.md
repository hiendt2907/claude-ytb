...
Khi cạn việc **hoặc** chạm ~98% giới hạn token của Codex → **pause** (không thoát
skill/prompt). Khi limit của Codex reset → **tự tiếp tục** đúng chỗ đang dở.
...
Hai module lo phần XÁC ĐỊNH của Bước -0.5; Codex chỉ lo phần sáng tạo (sinh 30 chủ đề,
...
- `ideation/series.py` — Bước B+C:
  - `rank_niches(candidates)` / `pick_niche(...)` — chấm tổng & chọn ngách (candidate do
    Codex chấm `{niche, scores:{search,competition,ypp,brand}}`; `derive_search_score`
    quy đổi views→điểm search gợi ý).
  - `dedup_topics(topics, ledger_text)` — lưới an toàn cấp slug, loại tập trùng ledger
    (chống-trùng NGỮ NGHĨA vẫn do Codex phán đoán theo Bước -2).
...
- **Lên lịch công khai**: nếu muốn video tự PUBLIC vào một mốc giờ → `YOUTUBE_PRIVACY=private`
  ...
  Publish thật xong vẫn **move file lên Drive** `Codex-YTB` rồi xoá local.
...
> Lưu ý cơ chế: khi Codex chạm limit cứng, phiên tự dừng và hiện giờ reset.
